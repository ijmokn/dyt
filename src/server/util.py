...
def get_env_file_path():
    ...
    For local development (RUNNING_IN_PRODUCTION not set):
      - First tries .azure/{defaultEnvironment}/.env (if .azure/config.json exists)
      - Falls back to project root .env file
    ...
    try:
        ...
        # If .azure config doesn't exist or env file not found, use project root .env
        root_env_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '.env'))
        if os.path.exists(root_env_file):
            return root_env_file
        
        # No .env file found, return None to use default
        return None
        
    except Exception as e:
        # On any error, try project root .env as fallback
        try:
            root_env_file = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '.env'))
            if os.path.exists(root_env_file):
                return root_env_file
